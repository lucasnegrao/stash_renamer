...
export const ListTable = <T extends { id: string }>(
  ...
  const [draggingColumnValue, setDraggingColumnValue] = useState<string | null>(null);
  ...
  useLayoutEffect(() => {
    setColumnWidths((prev) => {
      ...
      visibleColumns.forEach((column) => {
        ...
        const measured = column.resizable === false
  ...
  const baseTableWidth = useMemo(
    () =>
      ...
      visibleColumns.reduce(
        (acc, column) => acc + (baseColumnWidths[column.value] ?? MIN_COL_WIDTH),
        0
      ),
  ...
  const effectiveColumnWidths = useMemo(() => {
    ...
    if (missing > 0) {
      ...
      out[lastColumn.value] = (out[lastColumn.value] ?? MIN_COL_WIDTH) + missing;
    }
    return out;
      ...
      <tr key={item.id}>
        ...
        {visibleColumns.map((column) => (
          <td
            ...
            style={{ width: `${effectiveColumnWidths[column.value] ?? MIN_COL_WIDTH}px` }}
            ref={
              index === 0
              ...
              style={
                column.multiline === false
                  ...
                  : {
                      ...
                      WebkitLineClamp: column.maxLines ?? DEFAULT_MULTILINE_LINES,
                      minWidth: 0,
                    }
              }
            >
  ...
  const columnHeaders = useMemo(() => {
    return visibleColumns.map((column) => (
      <th
        ...
        onDragOver={(event) => {
          if (!draggingColumnValue || draggingColumnValue === column.value) return;
          event.preventDefault();
          event.dataTransfer.dropEffect = "move";
        }}
        onDrop={(event) => {
          event.preventDefault();
          const source = draggingColumnValue || event.dataTransfer.getData("text/plain");
          setDraggingColumnValue(null);
          if (!source) return;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <thead>
          <tr>
            <th className="select-col" style={{ width: `${SELECT_COL_WIDTH}px` }}>
              <div
