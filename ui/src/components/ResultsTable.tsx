...
function getStatusMeta(row: IScenePreviewResult): {
  ...
  const hasSuccess = successValue === true || successValue === 1 || successValue === "1";
  const hasFailure = successValue === false || successValue === 0 || successValue === "0";
  const detailsLower = details.toLowerCase();
  const isNoChange = detailsLower.includes("no change (same path and filename)");
  const isWarn = isNoChange || raw === "warn" || raw === "warning" || raw === "skipped";
  const isError = !isNoChange && (hasFailure || raw === "error" || raw === "fail");
  const isSuccess =
    hasSuccess ||
...
export const ResultsTable: React.FC<IResultsTableProps> = ({
  ...
  const allColumns: IColumnSpec[] = [
    ...
    },
    {
      ...
      maxLines: 2,
      render: (row) => (
        <span className="font-monospace" title={String(row.old_path || "")}>{String(row.old_path || "-")}</span>
      ),
    },
    {
      value: "new_path",
      ...
      maxLines: 2,
      render: (row) => (
        <span className="font-monospace" title={String(row.new_path || "")}>{String(row.new_path || "-")}</span>
      ),
    },
    {
      value: "status",
      ...
      render: (row) => {
        ...
        if (meta.tone === "success") {
          return <Icon icon={faCircleCheck} className="text-success" title={meta.details || meta.label} />;
        }
        if (meta.tone === "warning") {
          return <Icon icon={faTriangleExclamation} className="text-warning" title={meta.details || meta.label} />;
        }
        if (meta.tone === "danger") {
          return <Icon icon={faCircleXmark} className="text-danger" title={meta.details || meta.label} />;
        }
  ...
  const baseColumns: IColumn[] = allColumns.map(({ render, defaultShow, ...col }) => col);

  function renderCell(column: IColumn, row: IScenePreviewResult): React.ReactNode {
    const spec = allColumns.find((c) => c.value === column.value);
    return spec ? spec.render(row) : null;
  }

  if (rows.length === 0) {
    return <div className="text-muted text-center py-3">{loading ? "Loading..." : "No results found for this batch."}</div>;
  }
  ...
  return (
    <ListTable<IScenePreviewResult>
      ...
      items={rows.map((r, i) => ({
        ...r,
        id: String(r.id || `${r.scene_id || "scene"}-${r.created_at || "row"}-${i}`),
      }))}
