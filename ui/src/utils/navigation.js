...
import { ChildTagsCriterionOption, ParentTagsCriterionOption, TagsCriterion, TagsCriterionOption, } from "src/models/list-filter/criteria/tags";
import { ListFilterModel } from "src/models/list-filter/filter";
import { ContainingGroupsCriterionOption, GroupsCriterion, GroupsCriterionOption, SubGroupsCriterionOption, } from "src/models/list-filter/criteria/groups";
import { StringCriterion, createStringCriterionOption, } from "src/models/list-filter/criteria/criterion";
...
const makeDirectorScenesUrl = (director) => {
    if (director.length == 0)
        return "#";
    const filter = new ListFilterModel(GQL.FilterMode.Scenes, undefined);
    filter.criteria.push(stringEqualsCriterion(createStringCriterionOption("director"), director));
    return `/scenes?${filter.makeQueryParameters()}`;
};
...
const makePhotographerGalleriesUrl = (photographer) => {
    if (photographer.length == 0)
        return "#";
    const filter = new ListFilterModel(GQL.FilterMode.Galleries, undefined);
    filter.criteria.push(stringEqualsCriterion(createStringCriterionOption("photographer"), photographer));
    return `/galleries?${filter.makeQueryParameters()}`;
};
...
export function handleUnsavedChanges(intl, basepath, id) {
