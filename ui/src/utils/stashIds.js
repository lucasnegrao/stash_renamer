export const getStashIDs = (ids) => (ids !== null && ids !== void 0 ? ids : []).map(({ stash_id, endpoint, updated_at }) => ({
    stash_id,
    endpoint,
    updated_at,
}));
 ...
 */
export const separateNamesAndStashIds = (inputs) => {
    const names = [];
    const stashIds = [];
 ...
 */
export const addUpdateStashID = (existingStashIDs, newItem, allowMultiple = false) => {
