...
(function () {
  ...
  PluginApi.patch.after("SceneList", (props: any, _original: any, result: any) => {
    if (!patchesEnabled) return result;
    setSceneListRuntimeFromProps(props);
    return result;
  });

  PluginApi.patch.instead("SceneList", function (props: any, _arg:any, original: any) {
    if (typeof original === "function") {
      console.log(original);
      sceneListOriginalFn = original;
    }
    if (!patchesEnabled) {
      const fallback = typeof original === "function" ? original : sceneListOriginalFn;
      return fallback ? fallback(props) : null;
    }
    return (
      <div className="scene-list-table-root" style={{ width: "100%", maxWidth: "100%", minWidth: 0 }}>
        <SceneListTreeble {...props} sceneOperationById={getScenePreviewByIdState()} />
      </div>
    );
  });

  PluginApi.patch.instead("FilteredSceneList.SidebarSections", function (props: any, _arg:any, original: any) {
    if (typeof original === "function") {
  ...
  const MainPage: React.FC = () => {
    ...
    React.useEffect(() => {
      const styleId = "stash-renamer-layout-style";
    ...
    React.useLayoutEffect(() => {
      ...
      if (!pageEl) return;
      const navEl = pageEl.querySelector("#stash-renamer-tabs > .nav") as HTMLElement | null;
      if (!navEl) return;
    ...
    return (
      ...
      <div ref={pageRef} className="stash-renamer-page">
        <Tabs
          ...
          activeKey={tab}
          onSelect={(k: any) => setActiveTabState(String(k || "editor") as any)}
        >
