...
import { subscribeActiveTabState, subscribeResultsFocus } from "../services/renamerRuntimeState";
...
export const RenamerResults: React.FC = () => {
  ...
  const filteredOperations = React.useMemo(() => {
    ...
    return operations.filter((row) => {
      ...
      const unchangedByText = details.includes("no change (same path and filename)");
  ...
  async function loadBatches(selectLatest = true) {
    setLoading(true);
    try {
      const rows = (await fetchOperationBatches()).filter(
        (b) => String(b.mode || "").toLowerCase() !== "dry_run",
      );
      setBatches(rows);
    ...
    <div className="container-fluid p-8">
      <div className="d-flex align-items-center mb-2">
        ...
        <Button
          ...
          size="sm"
          onClick={onUndoBatch}
          disabled={loading || runningUndo || !selectedBatchId || selectedBatch?.mode !== "rename"}
        >
          {runningUndo ? (
            <>
